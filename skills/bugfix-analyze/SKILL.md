...
> Step 0 到此結束。觸發退出條件後，**立即停止**，不得繼續操作瀏覽器或讀取任何程式碼。完成截圖後進入下方義務表，再進入 Step 1。
...
| 失敗：操作完畢後未觀察到錯誤 | `reproduction-failed.png` | **預設 `already_fixed`**。靜態分析只允許用來「否定」此預設——即在完全符合 reproduction_steps 的程式碼路徑上找到明確邏輯錯誤。在其他地方找到「可能相關」的問題不算，不得以此將 Status 改為 `confirmed`。 |
| 失敗：auth / 頁面無法載入等環境問題 | `reproduction-failed.png` | 佐證缺失，繼續靜態 RCA，但 confidence 扣分（`靜態分析無法重現 -0.20`） |

> **原則**：「我完整操作了卻沒看到錯誤」是 already_fixed 的強佐證。LLM 不得以「程式碼看起來有問題」為由無視重現失敗的事實，強行輸出 `confirmed`。
